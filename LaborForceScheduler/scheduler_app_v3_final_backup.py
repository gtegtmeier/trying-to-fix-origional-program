...
def generate_schedule(model: DataModel, label: str,
    ...
    def add_best_segment(day: str, area: str, st: int, en: int, locked_ok: bool=False, enforce_weekly_cap: bool=True) -> bool:
        ...
        pool = [e for e in model.employees if e.work_status=="Active" and area in e.areas_allowed]
...
# Local search improvement (swap/move)
    ...
    def step(cur: List[Assignment]) -> List[Assignment]:
        ...
        if rnd.random() < 0.5:
            ...
            pool = [e for e in model.employees if e.work_status=="Active" and area in e.areas_allowed and e.name!=old.employee_name]
            rnd.shuffle(pool)
            for e2 in pool[:20]:
    ...
    min_req2, pref_req2, max_req2 = build_requirement_maps(model.requirements, goals=getattr(model,'manager_goals',None))
